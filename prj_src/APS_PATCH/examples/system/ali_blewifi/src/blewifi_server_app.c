...
static void BleWifi_Ble_AppMsgHandler_Disconnection(TASK task, MESSAGEID id, MESSAGE message);
...
static T_BleWifi_Ble_MsgHandlerTbl gBleAppMsgHandlerTbl[] =
{
    ...
    {BLEWIFI_APP_MSG_DISCONNECTION,             BleWifi_Ble_AppMsgHandler_Disconnection},
    ...
    if (BLEWIFI_BLE_DEVICE_NAME_METHOD == 1)
    {
    	...
    	if (LeGapGetBdAddr(addr) == SYS_ERR_SUCCESS)
    	{
    		...
    		*p++ = GAP_ADTYPE_LOCAL_NAME_COMPLETE;
            ...
            gTheBle.scn_data.buf[0] = gTheBle.scn_data.len - 1;     // update the total length, without buf[0]
...
static void BleWifi_Ble_CmMsgHandler_InitCompleteCfm(TASK task, MESSAGEID id, MESSAGE message)
{
    ...
    BleWifi_Ble_SetAdvtisingPara(LE_HCI_ADV_TYPE_ADV_IND,
                                 ...
                                 BLEWIFI_BLE_ADVERTISEMENT_INTERVAL_PS_MIN,
                                 BLEWIFI_BLE_ADVERTISEMENT_INTERVAL_PS_MAX);
}
...
{   
    ...
    LeGapDisconnectReq(gTheBle.conn_hdl);
}
