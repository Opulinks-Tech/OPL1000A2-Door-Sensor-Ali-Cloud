...
*/
SHM_DATA static void ikm_init(ali_init_t const *p_init)
{
...
ret_code_t ali_auth_init(ali_init_t const *p_init, tx_func_t tx_func)
{
    ...
    if(g_auth.device_secret_len == BZ_DEV_DEVICE_SECRET_LEN){
        ...
        BREEZE_ERR("\nAuth type not per product or per device %s\n", __func__);
...
int auth_calc_adv_sign(uint32_t seq, uint8_t *sign)
{
    ...
    sha256_update(&context, (const BYTE*)&seq, sizeof(seq));
