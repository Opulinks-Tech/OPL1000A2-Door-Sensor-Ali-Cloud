...
static int mbedtls_net_connect_ex( mbedtls_net_context *ctx, const char *host, const char *port, int proto ) //use true random port number
{
    int ret;
...
#else
    if (0 != (ret = mbedtls_net_connect_ex(&(pTlsData->fd), addr, port, MBEDTLS_NET_PROTO_TCP))) {
        printf(" failed ! net_connect returned -0x%04x\n", -ret);
        return ret;
    }
#endif
...
//    mbedtls_ssl_set_bio(&(pTlsData->ssl), &(pTlsData->fd), mbedtls_net_send, mbedtls_net_recv, mbedtls_net_recv_timeout);
    ...
    mbedtls_ssl_set_bio(&(pTlsData->ssl), &(pTlsData->fd), mbedtls_net_send_timeout, mbedtls_net_recv, mbedtls_net_recv_timeout);
...
static int ssl_poll_write(int fd, int timeout_ms)
{
    fd_set writeset;
    FD_ZERO(&writeset);
    FD_SET(fd, &writeset);
...
static int ssl_net_errno(int fd)
{
    int sock_errno = 0;
    u32_t optlen = sizeof(sock_errno);
...
static int _network_ssl_write(TLSDataParams_t *pTlsData, const char *buffer, int len, int timeout_ms)
{
    ...
    if ((poll = ssl_poll_write(pTlsData->fd.fd, timeout_ms)) <= 0) {
       printf("Poll timeout or error, errno=%d, fd=%d, timeout_ms=%d", ssl_net_errno (pTlsData->fd.fd), pTlsData->fd.fd, timeout_ms);
       return poll;
    }
