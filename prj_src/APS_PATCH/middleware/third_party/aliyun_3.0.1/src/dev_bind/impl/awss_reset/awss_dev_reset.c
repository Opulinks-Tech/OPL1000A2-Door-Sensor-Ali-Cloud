...
{
	hal_ali_netlink_Task_MsgSend(ALI_NETLINK_AWSS_REPORT_RESET_TO_CLOUD, NULL, 0);
}
...
//        report_reset_timer = HAL_Timer_Create("report_rst", (void (*)(void *))awss_report_reset_to_cloud, NULL);
        report_reset_timer = HAL_Timer_Create("report_rst", (void (*)(void *))awss_awss_report_reset_to_cloud_timer, NULL);
    }
    HAL_Timer_Stop(report_reset_timer);
