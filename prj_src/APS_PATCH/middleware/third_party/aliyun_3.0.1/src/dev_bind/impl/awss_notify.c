...
int awss_process_get_devinfo(void);
int awss_dev_bind_notify(void);

#ifdef ALI_SINGLE_TASK
#else
extern int hal_dev_info_lock(void);
extern int hal_dev_info_unlock(void);
#endif
...
int awss_notify_dev_info(int type, int count)
{
    ...
    do {
        ...
        buf = os_zalloc_(DEV_INFO_LEN_MAX);
        dev_info = os_zalloc_(DEV_INFO_LEN_MAX);
...
int awss_process_get_devinfo()	
{
    ...
    if (awss_report_token_suc == 0) {
        ...
        {
            timer_dbg("[%s %d] get_devinfo_timer is null\n", __func__, __LINE__);
        }
...
void awss_process_get_devinfo_timer(void)
{
	hal_ali_netlink_Task_MsgSend(ALI_NETLINK_AWSS_PROCESS_GET_DEVINFO, NULL, 0);
}


int online_get_device_info(void *ctx, void *resource, void *remote, void *request, char is_mcast)
{
    int iRet = -1;
    int timeout = 0;
...
//        get_devinfo_timer = HAL_Timer_Create("get_devinfo", (void (*)(void *))awss_process_get_devinfo, NULL);
	get_devinfo_timer = HAL_Timer_Create("get_devinfo", (void (*)(void *))awss_process_get_devinfo_timer, NULL);
    }
    HAL_Timer_Stop(get_devinfo_timer);
...
void awss_dev_bind_notify_timer(void)
{
	hal_ali_netlink_Task_MsgSend(ALI_NETLINK_AWSS_DEV_BIND_NOTIFY, NULL, 0);
}

void __awss_dev_bind_notify_timer(void)
{
	hal_ali_netlink_Task_MsgSend(ALI_NETLINK_AWSS_DEV_BIND_NOTIFY_FUN, NULL, 0);
}
...
//            printf("\r\nXXXdev_bind_cnt=%d\r\n", dev_bind_cnt);
//            printf("\r\nxxxawss_notify_resp[AWSS_NOTIFY_DEV_BIND_TOKEN]=%d\r\n", awss_notify_resp[AWSS_NOTIFY_DEV_BIND_TOKEN]);
            if (dev_bind_notify_timer == NULL) {				
//                dev_bind_notify_timer = HAL_Timer_Create("dev_bind", (void (*)(void *))awss_dev_bind_notify, NULL);
                dev_bind_notify_timer = HAL_Timer_Create("dev_bind", (void (*)(void *))awss_dev_bind_notify_timer, NULL);
            }
            HAL_Timer_Stop(dev_bind_notify_timer);
...
void send_awss_suc_notify()
{
	hal_ali_netlink_Task_MsgSend(ALI_NETLINK_AWSS_SUCCESS_NOTIFY, NULL, 0);
}

//static int __awss_suc_notify()
int __awss_suc_notify()
{
 ...
 //              success_notify_timer = HAL_Timer_Create("awss_suc", (void (*)(void *))__awss_suc_notify, NULL);
		success_notify_timer = HAL_Timer_Create("awss_suc", (void (*)(void *))send_awss_suc_notify, NULL);
            }
...
void awss_devinfo_notify_timer(void)
{
	hal_ali_netlink_Task_MsgSend(ALI_NETLINK_AWSS_DEV_INFO_NOTIFY, NULL, 0);
}
...
//                devinfo_notify_timer = HAL_Timer_Create("devinfo", (void (*)(void *))__awss_devinfo_notify, NULL);
	          devinfo_notify_timer = HAL_Timer_Create("devinfo", (void (*)(void *))awss_devinfo_notify_timer, NULL);
            }
