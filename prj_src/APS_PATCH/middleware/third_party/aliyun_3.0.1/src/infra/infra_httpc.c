...
int httpclient_connect_(httpclient_t *client)
{
    int retry_max = 3;
    int retry_cnt = 1;
...
int httpclient_recv_response_(httpclient_t *client, uint32_t timeout_ms, httpclient_data_t *client_data)
{
                      ...
                      HTTPCLIENT_REQUEST_TYPE method, httpclient_data_t *client_data)
{
    ...
    if (0 == client->net.handle) {
        ...
        ret = httpclient_connect_(client);
        if (0 != ret) {
...
int httpclient_common_(httpclient_t *client, const char *url, int port, const char *ca_crt,
