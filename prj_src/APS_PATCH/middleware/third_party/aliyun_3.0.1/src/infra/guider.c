...
extern int HAL_GetPartnerID(char *pid_str);
extern int HAL_GetModuleID(char *mid_str);

#define SYS_GUIDER_MALLOC_(size) HAL_Malloc(size)//LITE_malloc(size, MEM_MAGIC, "guider")
#define SYS_GUIDER_FREE(ptr) {HAL_Free((void *)ptr);ptr = NULL;}//LITE_free(ptr)
...
void iotx_conn_info_release_(void)
{
    if (iotx_conn_info.host_name != NULL) {
        HAL_Free(iotx_conn_info.host_name);
    }
    if (iotx_conn_info.username != NULL) {
        HAL_Free(iotx_conn_info.username);
    }
    if (iotx_conn_info.password != NULL) {
        HAL_Free(iotx_conn_info.password);
    }
    if (iotx_conn_info.client_id != NULL) {
        HAL_Free(iotx_conn_info.client_id);
    }
...
{
    iotx_mqtt_region_types_t region_type;
...
int iotx_guider_set_dynamic_region(int region)
{
    ...
    p_dynamic_mqtt_url = SYS_GUIDER_MALLOC_(GUIDER_URL_LEN);
                                ...
                                const char *timestamp_str, iotx_device_info_t *p_dev_info, ext_params_e ext_params)
{
    ...
    hmac_source_buf = SYS_GUIDER_MALLOC_(GUIDER_SIGN_SOURCE_LEN);
    if (!hmac_source_buf)
    {
        sys_err("no mem");
        return -1;
                          ...
                          const char *pkey)
{
    ...
    {
        ret = httpclient_common_(&httpc, url, port_num, pkey, HTTPCLIENT_GET,
    ...
    else
    {
        ...
        ret = httpclient_common_(&httpc, url, port_num, pkey, HTTPCLIENT_POST,
                                CONFIG_GUIDER_AUTH_TIMEOUT, &httpc_data);
    }

    httpclient_close_(&httpc);
...
static void guider_print_dev_guider_info(iotx_device_info_t *dev,
                                         ...
                                         char *guider_sign)
{
    char ds[IOTX_DEVICE_SECRET_LEN + 1];
    ...
    uint16_t *pport)
{
    ...
    */
    iotx_payload = SYS_GUIDER_MALLOC_(GUIDER_PREAUTH_RESPONSE_LEN);
...
static int get_itls_host_and_port(char *product_key, iotx_conn_info_t *conn)
{
    if (GUIDER_ENV_DAILY == iotx_guider_get_env())
    {
        ...
        conn->host_name = SYS_GUIDER_MALLOC_(sizeof(GUIDER_DIRECT_DOMAIN_ITLS_DAILY));
    ...
    {
        conn->port = 1883;
        conn->host_name = SYS_GUIDER_MALLOC_(sizeof(GUIDER_DIRECT_DOMAIN_ITLS_PRE));
        if (conn->host_name == NULL)
        {
            return -1;
        }
    ...
    else
    {
        ...
        conn->host_name = SYS_GUIDER_MALLOC_(host_name_len);
        if (conn->host_name == NULL)
        {
            return -1;
        }
...
static int get_non_itls_host_and_port(sdk_impl_ctx_t *ctx, char *product_key, iotx_conn_info_t *conn, char *p_dynamic_mqtt_url)
{
    ...
    {
        conn->port = 1883;
        conn->host_name = SYS_GUIDER_MALLOC_(sizeof(GUIDER_DIRECT_DOMAIN_DAILY));
        if (conn->host_name == NULL)
        {
            return -1;
        }
                                ...
                                char *p_guider_sign, secure_mode_e secure_mode, char *p_dynamic_mqtt_url)
{
    ...
    conn->username = SYS_GUIDER_MALLOC_(len);
    ...
    conn->password = SYS_GUIDER_MALLOC_(len);
    ...
    conn->client_id = SYS_GUIDER_MALLOC_(CLIENT_ID_LEN);
...
//    int len = 0;
    ...
    else
    {
        ...
        p_request_buf = SYS_GUIDER_MALLOC_(GUIDER_PREAUTH_REQUEST_LEN);
        if (!p_request_buf)
        {
            rc = -1;
            goto EXIT;
...
//        printf("\nreq_str = '%s'\n\n", p_request_buf);
    ...
    guider_print_dev_guider_info(p_dev, p_pid, p_mid, p_guider_url, secure_mode, timestamp_str, "no sign");
...
//    iotx_mqtt_region_types_t region_type;
    ...
    p_pid = SYS_GUIDER_MALLOC_(PID_STRLEN_MAX);
    if (!p_pid)
    {
        ret = -1;
        goto EXIT;
    ...
    p_mid = SYS_GUIDER_MALLOC_(MID_STRLEN_MAX);
    if (!p_mid)
    {
        ret = -1;
        goto EXIT;
    ...
    p_dev = SYS_GUIDER_MALLOC_(sizeof(iotx_device_info_t));
    if (!p_dev)
    {
        ret = -1;
        goto EXIT;
    ...
    p_guider_sign = SYS_GUIDER_MALLOC_(GUIDER_SIGN_LEN);
    if (!p_guider_sign)
    {
        ret = -1;
        goto EXIT;
    ...
    p_dynamic_mqtt_url = SYS_GUIDER_MALLOC_(GUIDER_DYNAMIC_URL_LEN);
    if (!p_dynamic_mqtt_url)
    {
        ret = -1;
        goto EXIT;
...
//            sys_info("dyna url:%s count:%d", p_dynamic_mqtt_url, direct_connect_count);
            printf("\ndyna url:%s count:%d\n\n", p_dynamic_mqtt_url, direct_connect_count);
    ...
    if (CONNECT_PREAUTH == connect_method)
    {
        p_guider_url = SYS_GUIDER_MALLOC_(GUIDER_URL_LEN);
        if (!p_guider_url)
        {
            ret = -1;
            goto EXIT;
