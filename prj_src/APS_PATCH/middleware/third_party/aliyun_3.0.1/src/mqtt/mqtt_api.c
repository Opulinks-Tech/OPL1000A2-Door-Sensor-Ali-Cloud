...
{
//    if (g_default_sign.hostname != NULL) {
//        HAL_Free(g_default_sign.hostname);
//    }
//    if (g_default_sign.username != NULL) {
//        HAL_Free(g_default_sign.username);
//    }
//    if (g_default_sign.password != NULL) {
//        HAL_Free(g_default_sign.password);
//    }
//    if (g_default_sign.client_id != NULL) {
//        HAL_Free(g_default_sign.client_id);
//    }
    memset(&g_default_sign, 0, sizeof(g_default_sign));
}
...
void *IOT_MQTT_Construct(iotx_mqtt_param_t *pInitParams)
{
    ...
    if (dynamic) {
        ...
        ret = _iotx_dynamic_register((iotx_http_region_types_t)region, (iotx_dev_meta_info_t *)&meta_info);
    ...
    ret = _iotx_preauth((iotx_mqtt_region_types_t)region, (iotx_dev_meta_info_t *)&meta_info, (iotx_sign_mqtt_t *)&g_default_sign); /* type convert */
    if (ret < SUCCESS_RETURN) {
...
#if 1   // the modified
    ret = IOT_Sign_MQTT((iotx_mqtt_region_types_t)region, &meta_info, &g_default_sign);
#else   // the original
